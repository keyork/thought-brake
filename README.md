...
当前已完成的主线实验覆盖 `math=100`、`mmlu=100`、`riddle=20`，每个策略配置共 220 条样本，使用 `direct` Phase 2，并记录 schema v3 token 字段。riddle 数据集已扩充到 100 条，下一轮 full run 会把 riddle 也纳入 100 题规模。报告入口是：

```bash
./experiments/run_token_main.sh
```

当前推荐默认策略是 **`compression@300`**：在质量接近最高点的前提下，拿到更高的 total token 节省。

| Config | Quality | Baseline | ReasoningSavings | TokenSavings | Lost | Phase2Fail |
|---|---:|---:|---:|---:|---:|---:|
| budget@300 | 88.0% | 98.4% | 78.1% | **38.7%** | 25 | 0.0% |
| budget@1000 | 93.9% | 98.4% | 42.9% | 21.7% | 12 | 0.0% |
| **compression@300** | **94.1%** | 97.0% | **63.3%** | **27.7%** | 10 | 0.0% |
| compression@1000 | 94.1% | 97.0% | 28.2% | 16.8% | 10 | 0.0% |
| keyword@300 | 90.0% | 96.6% | 63.2% | 28.5% | 20 | 0.0% |
| keyword@1000 | 95.0% | 96.6% | 25.5% | 17.3% | 7 | 0.0% |

分数据集的第一版路由建议：

| Dataset | Recommended | Quality | TokenSavings | 说明 |
|---|---|---:|---:|---|
| math | compression@300 | 93.0% | 25.5% | 比 1000-token 保守策略多省 token，质量仍在可接受区间 |
| mmlu | compression@300 | 97.0% | 31.1% | 当前质量和节省都最稳 |
| riddle | keyword@1000 | 100.0% | 18.9% | 样本只有 20 条，先按保守高质量策略处理 |

`TokenSavings` 使用 API `total_tokens`；流式 usage 不可用时按行回退到 `estimated_total_tokens`。本次 treated rows 中，304/1320 使用 API `total_tokens`，1016/1320 使用估算值。因此当前结论足以指导下一轮策略，但最终成本数字仍需继续提高 API token 覆盖率。
